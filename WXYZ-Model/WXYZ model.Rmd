...
#                                                                                #
#  PURPOSE: This is a deterministic version of the Markov model that is          #
...
```{r}
# The Excel file where the model inputs are stored
Source_Inputs <- "Model Inputs.xls"
...
# Toggle Probabilistic (TRUE) or Deterministic (FALSE)
Probabilistic = TRUE

# Strategy names
StratName_Ref <- "Standard_of_Care"
StratName_Exp <- "New_Treatment"
```
...
if (Probabilistic == TRUE){
  df_values <- varlist$df_psa_input} else 
    
  if (Probabilistic == FALSE){
  # A little work to transpose the deterministic data
  df_values <- as.data.frame(matrix(unlist(varlist$varmean), nrow = 1, byrow = TRUE))
  colnames(df_values) <- unlist(varlist$varname)
  n_sim <- 1}
